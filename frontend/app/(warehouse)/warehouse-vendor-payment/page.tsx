...
const WarehouseVendorPaymentPage: React.FC = () => {
  ...
  const fetchVendors = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/warehouse-vendors/viewvendor?page=1&limit=10000', {
        method: 'GET',
        credentials: 'include',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = `/api/warehouse-vendors/process-payment/${formData.vendor_id}`;
      ...
      if (response.ok) {
        ...
        Swal.fire({
          ...
          text: 'Warehouse vendor payment has been recorded successfully.',
          icon: 'success',
          timer: 2000,
  ...
  return (
    <div className="p-2 py-5 bg-white min-h-screen">
      <div className="max-w-[60%] mx-auto">
        <PageHeader title="Warehouse Vendor Payment" />
      </div>
      ...
      <div className="md:max-w-[60%] max-w-full mx-auto mt-6 rounded-lg shadow-lg p-2 py-5 md:p-6">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Vendor Dropdown */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Select Warehouse Vendor *</label>
            <select
              name="vendor_id"
...
export default WarehouseVendorPaymentPage;

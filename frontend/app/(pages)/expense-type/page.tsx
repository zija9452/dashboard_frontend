...
const ExpenseTypePage: React.FC = () => {
  ...
  const fetchExpenseTypes = async () => {
    try {
      ...
      const response = await fetch(`/api/expense-type/?${params.toString()}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      const total = data.total || expenseTypesList.length;
      ...
      showToast('Failed to fetch expense types', 'error');
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      if (editingExpenseType) {
        ...
        const response = await fetch(
          `/api/expense-type/${editingExpenseType.id}`,
          {
            method: 'PUT',
            headers: {
        ...
        Swal.fire({
          title: 'Updated!',
          text: 'Expense type has been updated successfully.',
          icon: 'success',
          timer: 2000,
        ...
        // Create expense type
        const response = await fetch('/api/expense-type/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        Swal.fire({
          title: 'Created!',
          text: 'Expense type has been created successfully.',
          icon: 'success',
          timer: 2000,
          timerProgressBar: true,
          showConfirmButton: false
      ...
      showToast(error instanceof Error ? error.message : 'Failed to save expense type', 'error');
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-4">
      <PageHeader title="Expense Type Management" />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {showAddForm ? 'Cancel' : '+ Add Expense Type'}
          </button>
        ...
        <div className="w-full sm:w-auto flex gap-2">
          ...
          <button
            ...
            onClick={() => {
              setSearchTerm('');
              document.getElementById('expenseTypeSearchInput')?.focus();
            }}
          >
            Search
          </button>
        ...
        <div className="border-0 p-0 mb-6 transition-all duration-300">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              >
                {submitting ? 'Saving...' : (editingExpenseType ? 'Update Expense Type' : 'Add Expense Type')}
              </button>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {expenseTypes.map((expenseType) => (
                  <tr key={expenseType.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{expenseType.name}</td>

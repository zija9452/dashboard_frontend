...
// This route handles /api/admin/updateadmin/[id]
export async function PUT(request: NextRequest) {
  try {
    // Extract the ID from the URL path
    const url = new URL(request.url);
    const pathParts = url.pathname.split('/');
    const userId = pathParts[pathParts.length - 1]; // Last part should be the ID
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/admin/updateadmin/${userId}`;
    ...
    // Forward the PUT request to the backend with JSON body

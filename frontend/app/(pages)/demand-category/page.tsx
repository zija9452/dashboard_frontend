...
const DemandCategoryPage: React.FC = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await fetch(`/api/demand-category/?${params.toString()}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      showToast('Failed to fetch demand categories', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        // Update category
        const response = await fetch(`/api/demand-category/${editingCategory.id}`, {
          method: 'PUT',
          headers: {
        ...
        Swal.fire({
          title: 'Updated!',
          text: 'Demand category has been updated successfully.',
          icon: 'success',
          timer: 2000,
        ...
        Swal.fire({
          title: 'Created!',
          text: 'Demand category has been created successfully.',
          icon: 'success',
          timer: 2000,
          timerProgressBar: true,
          showConfirmButton: false
      ...
      console.error('Error saving demand category:', error);
      showToast(error instanceof Error ? error.message : 'Failed to save demand category', 'error');
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      setDeletingId(id);
      try {
        const response = await fetch(`/api/demand-category/${id}`, {
          method: 'DELETE',
          credentials: 'include',
        });
        ...
        Swal.fire({
          title: 'Deleted!',
          text: 'Demand category has been deleted.',
          icon: 'success',
          timer: 2000,
          timerProgressBar: true,
          showConfirmButton: false
        ...
        setDeletingId(null);
        console.error('Error deleting demand category:', error);
        showToast('Failed to delete demand category', 'error');
      }
    }
  };
  ...
  return (
    <div className="p-2 py-5">
      <PageHeader title="Demand Category" />
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button
            ...
            onClick={() => {
              setSearchTerm('');
              setCurrentPage(1);
              document.getElementById('demandCategorySearchInput')?.focus();
            }}
          >
            Clear
          </button>
...
export default DemandCategoryPage;

...
// GET /api/shoporder/stream - Proxies the Shop Orders badge SSE stream
export async function GET(request: NextRequest) {
  const cookieHeader = request.headers.get('cookie') || '';
  const backendUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000'}/shoporder/stream`;

...
const VendorsPage: React.FC = () => {
  ...
  const [deletingId, setDeletingId] = useState<string | null>(null); // Track which vendor is being deleted
  ...
  const fetchVendors = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || errorData.error || 'Failed to fetch vendors');
      }
    ...
    try {
      ...
      if (editingVendor) {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          throw new Error(errorData.detail || errorData.error || 'Failed to update vendor');
        }
      } else {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          throw new Error(errorData.detail || errorData.error || 'Failed to create vendor');
        }
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          // Extract specific error message from detail or error fields
          const errorMessage = errorData.detail || errorData.error || 'Failed to delete vendor';

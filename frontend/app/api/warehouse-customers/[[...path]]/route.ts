...
async function proxyRequest(request: NextRequest, method: string, path: string) {
  try {
    ...
    const backendUrl = `${BACKEND_BASE_URL}/warehouse-customers${path}${searchParams ? '?' + searchParams : ''}`;
    ...
    console.error(`Error in warehouse-customers proxy (${method} ${path}):`, error);
    return Response.json(

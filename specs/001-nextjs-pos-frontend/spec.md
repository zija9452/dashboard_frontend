...
- Q: What are the exact authentication endpoint paths and methods for login, logout, and session validation? → A: Login: POST /auth/session-login with form data (username, password); Session validation: via cookie-based session management; Logout: Not explicitly defined, implemented as client-side session clearing with backend session management

- Q: What are the specific session cookie security settings (HttpOnly, Secure, SameSite, domain/path, TTL)? → A: Based on security documentation: HttpOnly=true, Secure=true, SameSite=Lax; Specific TTL not documented, assumed to follow standard session cookie behavior

- Q: What are the exact API endpoint paths and request/response schemas for the product management functionality? → A: Products: GET /products/, POST /products/, PUT /products/{id}, DELETE /products/{id}; Frontend-compatible: GET /products/get-products/{id}, GET /products/view-product; Complete CRUD documented in productapi.md with detailed request/response schemas

- Q: What are the specific permissions/roles required for different user types (admin/salesman/customer) and which endpoints they can access? → A: Admin: Full access to all operations; Cashier: Can view products, process customer invoices, create customers; Employee: Can view products, access basic operations, create vendors; Most endpoints enforce role-based access control via session cookies

- Q: What are the exact refund rules, allowed status transitions for invoices, and partial payment policies? → A: Refunds: POST /walkin-refund/refunds/walkin-invoice for walk-in invoices; Customer invoice refunds: PUT /customer-invoice/process-payment/{order_id}; Status transitions: issued → paid, partial, cancelled (documented in customer_invoice_api.md); Partial payments supported via payment processing endpoints